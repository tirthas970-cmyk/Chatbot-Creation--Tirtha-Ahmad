...
filename = "ChatBotSaver.txt" #defined globally 
#Save Data Function
def Imprint(document, log_user_prompt=False):  #takes an optional parameter to log initiaed user prompts
    ...
    log_entry = f"[{timestamp}] {document}\n" 
...
def Crosscheck(info):
    ...
    #check for key words (checker)
    keywords = set(wiki_info.lower().split())
    if any(word in wiki_info.lower() and word in ddg_text.lower() for word in keywords): #checks if similar words are found:
        verfied_info = f'Info verfied by other sources: {formatted_summary}'
        Imprint(f'Ai Assitant" {verfied_info}')
        return verfied_info
    else:
        contridicting = f'Contridicting Information. Wiki says {formatted_summary}\nDDG says {ddg_text}'
